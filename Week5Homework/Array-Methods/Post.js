...
const userFivePost = post.filter((task) => task.userId === 5);
console.log(userFivePost);

// (~~ Working Map Method ~~~)
const pairs = post.map((user) => {
...
// (~~~ Working For Each Method ~~~)

let titleBodyPairs = [];
post.forEach((task) => {
  let title = task.title;
  let body = task.body;
  return titleBodyPairs.push([{ title, body }]);
});
console.log(titleBodyPairs);

// (~~~ Working Arrow Syntax Method ~~~)

// let titleBodyPairs = [];
// post.forEach((task) =>
//   titleBodyPairs.push({ title: task.title, body: task.body })
// );
// console.log(titleBodyPairs);
